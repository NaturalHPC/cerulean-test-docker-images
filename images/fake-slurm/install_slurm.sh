...
apt-get update
apt-get --no-install-recommends install -y gcc make libssl-dev libmunge-dev tar \
    wget patch python3 python-is-python3 libmariadb-dev
...
echo 'Configuring...'
./configure --prefix=/usr/local --sysconfdir=/usr/local/etc/slurm --enable-multiple-slurmd CFLAGS="-g -O0"  >/tmp/configure.log 2>&1 || (cat /tmp/configure.log && exit 1)
...
apt-get purge -y gcc make wget libssl-dev libmunge-dev mariadb-client libmariadb-dev
